...
`vst-render` batch-renders VST presets to audio files using DawDreamer (a Python wrapper around JUCE) as a headless rendering engine. It supports two preset formats in v0.2.x: Serum 1's `.fxp` (loaded via `synth.load_preset`) and Serum 2's `.SerumPreset` (a cbor2+zstandard wrapper around the inner JUCE state blob, decoded by `serum2-preset-loader` then handed to `synth.load_state`). A worker pool (`loky`) runs one DawDreamer engine **per loaded format** in each process — fxp jobs render through their own engine, serum2 jobs through theirs, with no shared graph. Jobs dispatch to the correct engine based on the preset's file suffix.
...
- **One engine per loaded format per worker.** Multiple `RenderEngine` instances per worker can cause thread explosion and memory leaks (DawDreamer issues #88 + #1) — but that's specifically about creating engines *in a loop* or per-job, not about static creation. Two engines built once in `init_worker`, one per plugin, is safe. The earlier "both synths in one shared graph" design was retracted: `engine.load_graph` with multiple orphan source processors routes only the last one to `get_audio()`, silently dropping the others' audio. The "byte-identical silence" probe that originally validated the shared graph was tautological (see `docs/audit-log.md` 2026-05-20).
